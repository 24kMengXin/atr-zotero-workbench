# 历史原型设计：从 ATR 证据到可共同学习的研究工作台

> 此文描述的是早期只读投影原型，不再是产品契约。当前的正确边界、数据对象和验收闭环见 [重新定义的共同研究工作台](reframed-product-contract.md)。
...
MVP 从已有 `frontier-map` 的 tensions 创建问题节点；每个问题都连到锚定文献，但 UI 标明“由 ATR map 提出，非论文自身结论”。来源 ledger 会按 `source_kind` 保守分为 `scholarly_evidence`、`contextual_inspiration` 和 `source_needs_review`：新闻、技术博客、白皮书、社媒等只能启发现实问题，绝不自动升级为学术证据。UI 的“知识体系”视图从领域 → 研究问题 → 所需概念 → 锚定来源展开。
