...
| --- | --- | --- | --- |
| 先整理 `multilingual-aaai` 历史 auto-research 产物，能复用的理解后复用，错误/缺口清退或报告 | 部分完成 | [逐项归位矩阵](historical-program-disposition-matrix.md)覆盖 6 program/28 run；真正被当前链路消费的 164 个 artifact 已各自生成带原路径、SHA-256、consumer 与限制的只读 `LEGACY_MAPPED` sidecar；旧 run 未修改；391 条来源及 source/fulltext 缺口继续显式保留 | artifact 字节级消费谱系已补齐；仍需完成来源访问/全文核验和语义复审，不能把 sidecar 当作学术有效性证明 |
| 与更新后的 ATR v2 对齐，而不是把 v0.9 stage 伪装成 v2 历史 | 部分完成 | 一个 portfolio + 六个 child authority 均保持 `INTAKE`/v0；portfolio/child 各附加相应 mapping index（5/12 attachments），Zotero 历史区显示 28 run、164 sidecar 与消费边界；七个 authority controller check 通过 | posterior worker 不是独立 gate reviewer；六条仍需 owner/独立 reviewer 作 route/park/refresh 决策 |
| 插件使用 Zotero 原生 Collection、Reader、Note、Item Pane，不建立独立主界面 | 部分完成 | v0.6.6 的 paper 单一主动作打开 Zotero Reader，并把同一份来源 Note 作为可编辑 Item Pane section 与四区 ATR 定位同时放在右栏；两者分别折叠，ATR section header 可直接打开/固定同一 Note，深度编辑仍打开同一 Note；便携窗只是显式镜像，不拥有第二份状态；repo-local 三方案交互 fixture 把独立 dashboard 标为清退、Note tab 标为深度综合、Reader 双 section 标为日常主模式 | 2026-07-19 只读实查发现日常 profile 实际仍注册 0.5.2，尚未运行上述 v0.6.6 交互；需通过 Zotero 官方安装确认升级后再作体验验收 |
...
| 可安装候选 XPI | 部分完成 | GitHub Release v0.6.6 已发布；XPI SHA-256 `ce5df85d328b115e337d92be1a23cb3d3b5cc4db6d87b85d2d9ac6d423eaa829`；53 项测试与静态合同通过；新增历史消费 mapping Note、每个旧 run 的 sidecar 计数/决策/边界，以及 portfolio 图的 164 项汇总。v0.6.4 的隔离 Zotero 9 运行验证仍覆盖 106 个来源、12 个知识 Collection、16 个研究 Collection、Reader 双 section、annotation/typed owner input 和便携四区/双图。XPI 连续两次构建字节完全一致；更新清单使用 SHA-256 且不打进 XPI | 仍需日常 profile 人工走查折叠密度与实际长文阅读体验 |
...
3. 在日常 Zotero 用当前 v0.6.6 候选人工验收 portfolio program 单击、问题卡→碰撞复核子 Note、owner route review，以及 paper 的「阅读原文并记录我的理解」是否让阅读 Note 与 ATR 定位在同一右栏同时可见。
