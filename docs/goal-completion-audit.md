...
| 可安装候选 XPI | 部分完成 | `dist/atr-zotero-workbench.xpi` v0.6.5，SHA-256 `9d1b3f1a80cf82da1f1d550ed7d89b35ebc6772dd228db6253cdc8d1dd8cf63d`；51 项测试与静态合同通过；v0.6.4 的隔离 Zotero 9 运行验证仍覆盖 106 个来源、12 个知识 Collection、16 个研究 Collection、Reader 双 section、annotation/typed owner input 和便携四区/双图；v0.6.5 在相同原生合同上新增 ATR section header 的同一 Note 动作，并以 repo-local 三方案 fixture 验证两 section、四个折叠区和便携只读镜像的布局语义。XPI 使用固定文件顺序和 `zip -X`，连续两次构建字节完全一致；远程 `update.json` 指向带 SHA-256 的 GitHub Release，更新清单不打进 XPI，避免自引用 archive hash | v0.6.5 尚需发布，随后仍需日常 profile 人工走查折叠密度与实际长文阅读体验 |
