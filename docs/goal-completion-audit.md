...
| 与更新后的 ATR v2 对齐，而不是把 v0.9 stage 伪装成 v2 历史 | 部分完成 | 一个 portfolio + 六个 child authority 均保持 `INTAKE`/v0；portfolio/child 各附加 mapping index 和 digest-versioned Zotero availability audit（6/13 attachments），Zotero 历史区显示 28 run、164 sidecar、来源可用性及边界；七个 authority controller check 通过 | posterior worker 不是独立 gate reviewer；六条仍需 owner/独立 reviewer 作 route/park/refresh 决策 |
| 插件使用 Zotero 原生 Collection、Reader、Note、Item Pane，不建立独立主界面 | 部分完成 | v0.6.7 的 paper 单一主动作打开 Zotero Reader，并把同一份来源 Note 作为可编辑 Item Pane section 与四区 ATR 定位同时放在右栏；两者分别折叠；来源对账也作为只读 Zotero Note/折叠 Item Pane section 显示，严格匹配的来源可直接回到 Reader；便携窗只是显式镜像，不拥有第二份状态 | 2026-07-19 只读实查发现日常 profile 实际仍注册 0.5.2，尚未运行上述 v0.6.7 交互；需通过 Zotero 官方安装确认升级后再作体验验收 |
...
| 可安装候选 XPI | 部分完成 | GitHub Release v0.6.7 已发布；XPI SHA-256 `1dd208d7a5dc95f335713ad3b2329af830e0aad5e8e33e5162a010b03c6de8ac`；65 项测试与静态合同通过；除 28 run/164 sidecar 历史视图外，新增 344 canonical source × 日常 Zotero 对账 Note、7 个本地 PDF 状态和严格的 availability≠inspection 边界。XPI 连续两次构建字节一致；更新清单使用 SHA-256 且不打进 XPI | 在日常 profile 完成一次实际共读验收 |
...
3. 在日常 Zotero 用当前 v0.6.7 候选人工验收 portfolio program 单击、问题卡→碰撞复核子 Note、owner route review，以及 paper 的「阅读原文并记录我的理解」是否让阅读 Note 与 ATR 定位在同一右栏同时可见。
