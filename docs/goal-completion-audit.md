...
| 可安装候选 XPI | 部分完成 | `dist/atr-zotero-workbench.xpi` v0.6.4，SHA-256 `a3ca9feee8bfb01acaf755a9ee084eff47ec29ffb7f2251e1796809447c1c79e`；50 项测试与静态合同通过；隔离 Zotero 9 验证 106 个来源、12 个知识 Collection、16 个研究 Collection、1 个现实证据缺口、1 个碰撞复核 Note；Reader 保持 `item` mode，真正的来源 `note-editor` section 与 ATR section 同时注册并分别折叠，同一 Note 仍可开原生标签，annotation/typed owner input 形成 3 个 review-only packet，便携镜像仍有四区/双图。portfolio smoke 另验证 6 program、4 折叠面板、1 张 4/2 disposition 总览、折叠 owner-review 队列、program→child→exact collision-review Note→portfolio 往返且 authority effect 为 NONE。远程 `update.json` 指向带 SHA-256 的 GitHub Release，旧版本可由 Zotero 的检查更新流程升级；更新清单不打进 XPI，避免自引用 archive hash | 仍需日常 profile 人工走查折叠密度与实际长文阅读体验 |
