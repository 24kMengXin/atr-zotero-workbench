...
| 插件使用 Zotero 原生 Collection、Reader、Note、Item Pane，不建立独立主界面 | 部分完成 | v0.6.5 的 paper 单一主动作打开 Zotero Reader，并把同一份来源 Note 作为可编辑 Item Pane section 与四区 ATR 定位同时放在右栏；两者分别折叠，ATR section header 可直接打开/固定同一 Note，深度编辑仍打开同一 Note；便携窗只是显式镜像，不拥有第二份状态；repo-local 三方案交互 fixture 把独立 dashboard 标为清退、Note tab 标为深度综合、Reader 双 section 标为日常主模式 | 尚需日常 profile 人工走查折叠密度与实际长文阅读体验 |
...
| 可安装候选 XPI | 部分完成 | GitHub Release v0.6.5 已发布；`dist/atr-zotero-workbench.xpi` SHA-256 `9d1b3f1a80cf82da1f1d550ed7d89b35ebc6772dd228db6253cdc8d1dd8cf63d`；51 项测试与静态合同通过；v0.6.4 的隔离 Zotero 9 运行验证仍覆盖 106 个来源、12 个知识 Collection、16 个研究 Collection、Reader 双 section、annotation/typed owner input 和便携四区/双图；v0.6.5 在相同原生合同上新增 ATR section header 的同一 Note 动作，并以 repo-local 三方案 fixture 验证两 section、四个折叠区和便携只读镜像的布局语义。XPI 使用固定文件顺序和 `zip -X`，连续两次构建字节完全一致；远程 `update.json` 指向带 SHA-256 的 GitHub Release，更新清单不打进 XPI，避免自引用 archive hash | 仍需日常 profile 人工走查折叠密度与实际长文阅读体验 |
