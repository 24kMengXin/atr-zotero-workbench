...
| HZ-009 | 历史 multilingual continuation 是 v0.9 `R2_FOCUSED_REVIEW`，不能把其旧 stage 直接写入 v2 SQLite；但其中的 baseline 与 source-grounded concept map 有复用价值 | 若直接“迁移阶段”，会伪造 v2 的 transition 和 current claim；若完全不带历史材料，新 controller 又失去可阅读的起点 | 新建 `2026-07-18-multilingual-agent-action-v2-intake`：只注册历史 baseline 与两条明确来源支撑的 knowledge-map，subject 停在 `INTAKE`/version 0。controller check 通过；workbench 从 SQLite 直接投影 2 个来源、6 个概念和 1 个非授权 attachment。 |
